...
def main(exp_name, batch_size, model_name, base_dir = "output"):
    ...
    # qualcom has different precision current results are for float16 
    pipe = StableDiffusionPipeline.from_pretrained(model_name,
                                                  torch_dtype=torch.float16,# Full Precision floa32
    ...
    for index, (prompts) in enumerate(tqdm(train_dataloader)):
        ...
        generated_image.save("stable_diff_image_"+str(count)+".png")
        count+=1
    prompt_list = pd.DataFrame(prompt_list)
    prompt_list.to_csv("stable_diff_prompt_list.csv")
